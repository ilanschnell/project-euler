...
for i in range(n):
    for j in range(n):
        w = matrix[i][j]
        G.add_edge((i, j - 1), (i, j), weight=w)  # right
        G.add_edge((i - 1, j), (i, j), weight=w)  # down
...
for i in range(n):
    ...
    for j in range(n):
        G.add_edge((i + 1, j), (i, j), weight=matrix[i][j])  # up
...
for i in range(n):
    for j in range(n):
        G.add_edge((i, j + 1), (i, j), weight=matrix[i][j])  # left
